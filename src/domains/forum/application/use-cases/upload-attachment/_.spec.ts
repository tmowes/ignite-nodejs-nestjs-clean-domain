...
describe('Upload and create attachment', () => {
  ...
  it('should be able to upload and create an attachment', async () => {
    ...
    expect(result.value).toEqual({ attachment: inMemoryAttachmentsRepository.items[0] })
    expect(fakeUploader.uploads).toHaveLength(1)
    expect(fakeUploader.uploads[0]).toEqual(expect.objectContaining({ fileName: 'profile.png' }))
  })
